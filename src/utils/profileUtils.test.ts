...
describe("getEditableFields cases", () => {
  ...
  it("should allow an Admin to edit their own firstName and lastName", () => {
    ...
    expect(utils.getEditableFields(admin, admin, "profile").sort()).toEqual(
      expected
    );
  });
  ...
  it("should allow an Admin to edit a Admin's role, organization, userStatus", () => {
    ...
    expect(
      utils
        .getEditableFields(admin, { ...admin, _id: "AAA-123-19291" }, "users")
        .sort()
    ).toEqual(expected);
  });
  ...
  it("should allow an Admin to edit a User's role, organization, userStatus", () => {
    ...
    expect(utils.getEditableFields(admin, user, "users").sort()).toEqual(
      expected
    );
    expect(utils.getEditableFields(admin, orgOwner, "users").sort()).toEqual(
      expected
    );
  });
  ...
  it("should allow an Admin to edit a Federal Lead's role, userStatus, and organization", () => {
    ...
    expect(utils.getEditableFields(admin, federalLead, "users").sort()).toEqual(
      expected
    );
  });
  ...
  it("should allow an Org Owner to edit their own firstName and lastName", () => {
    const expected = ["firstName", "lastName"].sort();
    expect(
      utils.getEditableFields(orgOwner, orgOwner, "profile").sort()
    ).toEqual(expected);
  });

  it("should allow an Org Owner to view only", () => {
    const expected = [].sort();
    expect(utils.getEditableFields(orgOwner, user, "users").sort()).toEqual(
      expected
    );
  });
  ...
  it("should allow Federal Lead to edit their own firstName and lastName", () => {
    const expected = ["firstName", "lastName"].sort();
    expect(
      utils.getEditableFields(federalLead, federalLead, "profile").sort()
    ).toEqual(expected);
  });

  it("should allow User's to edit their own firstName and lastName", () => {
    const expected = ["firstName", "lastName"].sort();
    expect(utils.getEditableFields(user, user, "profile").sort()).toEqual(
      expected
    );
  });
  ...
  it("by default, should not allow Federal Lead to edit another user's fields", () => {
    ...
    expect(
      utils.getEditableFields(
        federalLead,
        { ...federalLead, _id: "ABC-NOT-MY-ID" },
        "users"
      )
    ).toEqual([]);
  });
  ...
  it("by default, should not allow User to edit another user's fields", () => {
    ...
    expect(
      utils.getEditableFields(user, { ...user, _id: "ABC-NOT-MY-ID" }, "users")
    ).toEqual([]);
  });
});
